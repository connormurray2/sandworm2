...
var _bip39EnWordlist = require("./bip-39-en-wordlist");

var _workers = require("./workers");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

class Bip39 extends _struct.Struct {
  constructor(mnemonic, seed) {
    let wordlist = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : _bip39EnWordlist.wordList;
    super({
      mnemonic,
      seed
    });
  ...
  fromRandom(bits) {
    ...
    const buf = _random.Random.getRandomBuffer(bits / 8);
  ...
  async asyncFromRandom(bits) {
    ...
    let workersResult = await _workers.Workers.asyncObjectMethod(this, 'entropy2Mnemonic', [buf]);
    const bip39 = new Bip39().fromFastBuffer(workersResult.resbuf);
    workersResult = await _workers.Workers.asyncObjectMethod(bip39, 'mnemonic2Seed', []);
  ...
  async asyncFromEntropy(buf) {
    const workersResult = await _workers.Workers.asyncObjectMethod(this, 'fromEntropy', [buf]);
    return this.fromFastBuffer(workersResult.resbuf);
  }
  ...
  async asyncToSeed(passphrase) {
    ...
    const workersResult = await _workers.Workers.asyncObjectMethod(this, 'toSeed', args);
  ...
  entropy2Mnemonic(buf) {
    ...
    const hash = _hash.Hash.sha256(buf);
