...
class Msg extends _struct.Struct {
  ...
  static checksum(dataBuf) {
    return _hash.Hash.sha256Sha256(dataBuf).slice(0, 4);
  }

  static async asyncChecksum(dataBuf) {
    const hashBuf = await _hash.Hash.asyncSha256Sha256(dataBuf);
  ...
  *genFromBuffers(opts) {
    ...
    this.magicNum = new _br.Br(res.buf).readUInt32BE();
    ...
    this.cmdbuf = new _br.Br(res.buf).read(12);
    res = yield* this.expect(4, res.remainderbuf);
    this.datasize = new _br.Br(res.buf).readUInt32BE();
    ...
    res = yield* this.expect(4, res.remainderbuf);
    this.checksumbuf = new _br.Br(res.buf).read(4);
    res = yield* this.expect(this.datasize, res.remainderbuf);
    this.dataBuf = new _br.Br(res.buf).read(this.datasize);
  ...
  isValid() {
    ...
    return (0, _cmp.cmp)(checksumbuf, this.checksumbuf);
  }
