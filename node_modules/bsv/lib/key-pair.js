...
class KeyPair extends _struct.Struct {
  constructor(privKey, pubKey) {
    let PrivKey = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : _privKey.PrivKey;
    super({
      privKey,
      pubKey
    });
  ...
  fromJSON(json) {
    ...
    if (json.pubKey) {
      this.pubKey = _pubKey.PubKey.fromJSON(json.pubKey);
    }
  ...
  fromBr(br) {
    ...
    if (buflen2 > 0) {
      this.pubKey = new _pubKey.PubKey().fromFastBuffer(br.read(buflen2));
    }
  ...
  fromPrivKey(privKey) {
    this.privKey = privKey;
    this.pubKey = new _pubKey.PubKey().fromPrivKey(privKey);
    return this;
  }
  ...
  async asyncFromPrivKey(privKey) {
    this.privKey = privKey;
    this.pubKey = await new _pubKey.PubKey().asyncFromPrivKey(privKey);
    return this;
  }
...
KeyPair.Mainnet = class extends KeyPair {
  constructor(privKey, pubKey) {
    super(privKey, pubKey, _privKey.PrivKey.Mainnet);
  }

};
KeyPair.Testnet = class extends KeyPair {
  constructor(privKey, pubKey) {
    super(privKey, pubKey, _privKey.PrivKey.Testnet);
  }
