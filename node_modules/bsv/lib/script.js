...
class Script extends _struct.Struct {
  constructor() {
  ...
  fromBuffer(buf) {
    ...
    while (!br.eof()) {
      ...
      if (opCodeNum > 0 && opCodeNum < _opCode.OpCode.OP_PUSHDATA1) {
        len = opCodeNum;
        this.chunks.push({
          buf: br.read(len),
          len: len,
      ...
      } else if (opCodeNum === _opCode.OpCode.OP_PUSHDATA1) {
        try {
          len = br.readUInt8();
          buf = br.read(len);
        } catch (err) {
      ...
      } else if (opCodeNum === _opCode.OpCode.OP_PUSHDATA2) {
        try {
          len = br.readUInt16LE();
          buf = br.read(len);
        } catch (err) {
      ...
      } else if (opCodeNum === _opCode.OpCode.OP_PUSHDATA4) {
        try {
          len = br.readUInt32LE();
          buf = br.read(len);
        } catch (err) {
  ...
  toBuffer() {
    ...
    for (let i = 0; i < this.chunks.length; i++) {
      ...
      if (chunk.buf) {
        if (opCodeNum < _opCode.OpCode.OP_PUSHDATA1) {
          bw.write(chunk.buf);
        } else if (opCodeNum === _opCode.OpCode.OP_PUSHDATA1) {
  ...
  fromString(str) {
    ...
    while (i < tokens.length) {
      ...
      try {
        const opCode = new _opCode.OpCode().fromString(token);
      ...
      } else if (opCodeNum === _opCode.OpCode.OP_PUSHDATA1 || opCodeNum === _opCode.OpCode.OP_PUSHDATA2 || opCodeNum === _opCode.OpCode.OP_PUSHDATA4) {
  ...
  toString() {
    ...
    for (let i = 0; i < this.chunks.length; i++) {
      ...
      if (!chunk.buf) {
        if (_opCode.OpCode.str[opCodeNum] !== undefined) {
          str = str + ' ' + new _opCode.OpCode(opCodeNum).toString();
        } else {
          str = str + ' ' + '0x' + opCodeNum.toString(16);
        }
      } else {
        if (opCodeNum === _opCode.OpCode.OP_PUSHDATA1 || opCodeNum === _opCode.OpCode.OP_PUSHDATA2 || opCodeNum === _opCode.OpCode.OP_PUSHDATA4) {
          str = str + ' ' + new _opCode.OpCode(opCodeNum).toString();
        }
  ...
  fromBitcoindString(str) {
    ...
    for (i = 0; i < tokens.length; i++) {
      ...
      if (token[0] === '0' && token[1] === 'x') {
        ...
        bw.write(tbuf);
      } else if (_opCode.OpCode['OP_' + token] !== undefined) {
        const opstr = 'OP_' + token;
        const opCodeNum = _opCode.OpCode[opstr];
        bw.writeUInt8(opCodeNum);
      } else if (typeof _opCode.OpCode[token] === 'number') {
        const opstr = token;
        const opCodeNum = _opCode.OpCode[opstr];
        bw.writeUInt8(opCodeNum);
      } else if (!isNaN(parseInt(token, 10))) {
        const bn = new _bn.Bn(token);
  ...
  toBitcoindString() {
    ...
    for (let i = 0; i < this.chunks.length; i++) {
      ...
      if (chunk.buf) {
        ...
        str = str + ' ' + '0x' + hex;
      } else if (_opCode.OpCode.str[chunk.opCodeNum] !== undefined) {
        const ostr = new _opCode.OpCode(chunk.opCodeNum).toString();
  ...
  fromAsmString(str) {
    ...
    while (i < tokens.length) {
      ...
      try {
        opCode = _opCode.OpCode.fromString(token);
        opCodeNum = opCode.toNumber();
      } catch (err) {
        opCode = undefined;
        opCodeNum = undefined;
      ...
      if (token === '0') {
        ...
        opCodeNum = _opCode.OpCode.OP_1NEGATE;
        this.chunks.push({
          opCodeNum: opCodeNum
        });
        i = i + 1;
        ...
        if (len >= 0 && len < _opCode.OpCode.OP_PUSHDATA1) {
          opCodeNum = len;
        } else if (len < Math.pow(2, 8)) {
          opCodeNum = _opCode.OpCode.OP_PUSHDATA1;
        } else if (len < Math.pow(2, 16)) {
          opCodeNum = _opCode.OpCode.OP_PUSHDATA2;
        } else if (len < Math.pow(2, 32)) {
          opCodeNum = _opCode.OpCode.OP_PUSHDATA4;
        }
  ...
  _chunkToString(chunk, type) {
    ...
    if (!chunk.buf) {
      // no data chunk
      if (typeof _opCode.OpCode.str[opCodeNum] !== 'undefined') {
  ...
  fromOpReturnData(dataBuf) {
    this.writeOpCode(_opCode.OpCode.OP_RETURN);
  ...
  fromSafeData(dataBuf) {
    this.writeOpCode(_opCode.OpCode.OP_FALSE);
    this.writeOpCode(_opCode.OpCode.OP_RETURN);
    this.writeBuffer(dataBuf);
    return this;
  }
  ...
  fromPubKeyHash(hashBuf) {
    ...
    this.writeOpCode(_opCode.OpCode.OP_DUP);
    this.writeOpCode(_opCode.OpCode.OP_HASH160);
    this.writeBuffer(hashBuf);
    this.writeOpCode(_opCode.OpCode.OP_EQUALVERIFY);
    this.writeOpCode(_opCode.OpCode.OP_CHECKSIG);
    return this;
  }
  ...
  fromPubKeys(m, pubKeys) {
    ...
    this.writeOpCode(m + _opCode.OpCode.OP_1 - 1);

    for (const i in pubKeys) {
      this.writeBuffer(pubKeys[i].toBuffer());
    }

    this.writeOpCode(pubKeys.length + _opCode.OpCode.OP_1 - 1);
    this.writeOpCode(_opCode.OpCode.OP_CHECKMULTISIG);
    return this;
  }
  ...
  removeCodeseparators() {
    ...
    for (let i = 0; i < this.chunks.length; i++) {
      if (this.chunks[i].opCodeNum !== _opCode.OpCode.OP_CODESEPARATOR) {
  ...
  isPushOnly() {
    for (let i = 0; i < this.chunks.length; i++) {
      ...
      if (opCodeNum > _opCode.OpCode.OP_16) {
        return false;
      }
    }
  ...
  isOpReturn() {
    if (this.chunks[0].opCodeNum === _opCode.OpCode.OP_RETURN && this.chunks.filter(chunk => Buffer.isBuffer(chunk.buf)).length === this.chunks.slice(1).length) {
      return true;
    } else {
      return false;
    }
  ...
  isSafeDataOut() {
    ...
    if (this.chunks[0].opCodeNum !== _opCode.OpCode.OP_FALSE) {
      return false;
    }
  ...
  isPubKeyHashOut() {
    if (this.chunks[0] && this.chunks[0].opCodeNum === _opCode.OpCode.OP_DUP && this.chunks[1] && this.chunks[1].opCodeNum === _opCode.OpCode.OP_HASH160 && this.chunks[2].buf && this.chunks[3] && this.chunks[3].opCodeNum === _opCode.OpCode.OP_EQUALVERIFY && this.chunks[4] && this.chunks[4].opCodeNum === _opCode.OpCode.OP_CHECKSIG) {
      return true;
    } else {
      return false;
    }
  ...
  isPubKeyHashIn() {
    if (this.chunks.length === 2 && (this.chunks[0].buf || this.chunks[0].opCodeNum === _opCode.OpCode.OP_0) && this.chunks[1].buf) {
      return true;
    } else {
      return false;
    }
  ...
  isScriptHashOut() {
    ...
    return buf.length === 23 && buf[0] === _opCode.OpCode.OP_HASH160 && buf[1] === 0x14 && buf[22] === _opCode.OpCode.OP_EQUAL;
  }
  /**
  ...
  isMultiSigOut() {
    const m = this.chunks[0].opCodeNum - _opCode.OpCode.OP_1 + 1;
    ...
    if (!pubKeychunks.every(chunk => {
      try {
        const buf = chunk.buf;
        const pubKey = new _pubKey.PubKey().fromDer(buf);
        pubKey.validate();
        return true;
      } catch (err) {
        return false;
    ...
    const n = this.chunks[this.chunks.length - 2].opCodeNum - _opCode.OpCode.OP_1 + 1;

    if (!(n >= m && n <= 16)) {
      return false;
    }

    if (this.chunks[1 + n + 1].opCodeNum !== _opCode.OpCode.OP_CHECKMULTISIG) {
      return false;
    }
  ...
  isMultiSigIn() {
    if (this.chunks[0].opCodeNum !== _opCode.OpCode.OP_0) {
      return false;
    }
    ...
    return remaining.every(chunk => Buffer.isBuffer(chunk.buf) && _sig.Sig.IsTxDer(chunk.buf));
  }
  /**
  ...
  writeBn(bn) {
    if (bn.cmp(0) === _opCode.OpCode.OP_0) {
      this.chunks.push({
        opCodeNum: _opCode.OpCode.OP_0
      });
    } else if (bn.cmp(-1) === 0) {
      this.chunks.push({
        opCodeNum: _opCode.OpCode.OP_1NEGATE
      ...
      // see OP_1 - OP_16
      this.chunks.push({
        opCodeNum: bn.toNumber() + _opCode.OpCode.OP_1 - 1
      });
    } else {
      const buf = bn.toSm({
        endian: 'little'
  ...
  writeNumber(number) {
    this.writeBn(new _bn.Bn().fromNumber(number));
    return this;
  }
  ...
  writeBuffer(buf) {
    ...
    if (buf.length > 0 && buf.length < _opCode.OpCode.OP_PUSHDATA1) {
  ...
  checkMinimalPush(i) {
    ...
    if (buf.length === 0) {
      ...
      return opCodeNum === _opCode.OpCode.OP_0;
    } else if (buf.length === 1 && buf[0] >= 1 && buf[0] <= 16) {
      // Could have used OP_1 .. OP_16.
      return opCodeNum === _opCode.OpCode.OP_1 + (buf[0] - 1);
    } else if (buf.length === 1 && buf[0] === 0x81) {
      // Could have used OP_1NEGATE.
      return opCodeNum === _opCode.OpCode.OP_1NEGATE;
    } else if (buf.length <= 75) {
      // Could have used a direct push (opCode indicating number of bytes pushed + those bytes).
      return opCodeNum === buf.length;
    } else if (buf.length <= 255) {
      // Could have used OP_PUSHDATA.
      return opCodeNum === _opCode.OpCode.OP_PUSHDATA1;
    } else if (buf.length <= 65535) {
      // Could have used OP_PUSHDATA2.
      return opCodeNum === _opCode.OpCode.OP_PUSHDATA2;
    }
