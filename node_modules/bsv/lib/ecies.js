...
Ecies.ivkEkM = function (privKey, pubKey) {
  ...
  const P = KB.mul(r);
  const S = new _pubKey.PubKey(P);
  const Sbuf = S.toBuffer();

  const hash = _hash.Hash.sha512(Sbuf);
...
Ecies.electrumEncrypt = function (messageBuf, toPubKey, fromKeyPair) {
  ...
  const ciphertext = _aescbc.Aescbc.encrypt(messageBuf, kE, iv, false);

  const BIE1 = Buffer.from('BIE1');
  const encBuf = Buffer.concat([BIE1, Rbuf, ciphertext]);

  const hmac = _hash.Hash.sha256Hmac(encBuf, kM);
...
Ecies.electrumDecrypt = function (encBuf, toPrivKey) {
  ...
  const fromPubKey = _pubKey.PubKey.fromDer(pub);
  ...
  const hmac2 = _hash.Hash.sha256Hmac(encBuf.slice(0, encBuf.length - tagLength), kM);

  if (!hmac.equals(hmac2)) {
    throw new Error('Invalid checksum');
  }

  return _aescbc.Aescbc.decrypt(ciphertext, kE, iv);
};

Ecies.bitcoreEncrypt = function (messageBuf, toPubKey, fromKeyPair, ivBuf) {
  if (!fromKeyPair) {
    fromKeyPair = _keyPair.KeyPair.fromRandom();
  }
  ...
  const kEkM = _hash.Hash.sha512(Sbuf);

  const kE = kEkM.slice(0, 32);
  const kM = kEkM.slice(32, 64);

  const c = _aescbc.Aescbc.encrypt(messageBuf, kE, ivBuf);

  const d = _hash.Hash.sha256Hmac(c, kM);
...
Ecies.asyncBitcoreEncrypt = async function (messageBuf, toPubKey, fromKeyPair, ivBuf) {
  if (!fromKeyPair) {
    fromKeyPair = await _keyPair.KeyPair.asyncFromRandom();
  }
  ...
  const workersResult = await _workers.Workers.asyncClassMethod(Ecies, 'bitcoreEncrypt', args);
...
Ecies.bitcoreDecrypt = function (encBuf, toPrivKey) {
  ...
  const fromPubKey = _pubKey.PubKey.fromDer(encBuf.slice(0, 33));

  const R = fromPubKey.point;
  const P = R.mul(kB);

  if (P.eq(new _point.Point())) {
  ...
  const d2 = _hash.Hash.sha256Hmac(c, kM);

  if (!(0, _cmp.cmp)(d, d2)) {
    throw new Error('Invalid checksum');
  }

  const messageBuf = _aescbc.Aescbc.decrypt(c, kE);
...
Ecies.asyncBitcoreDecrypt = async function (encBuf, toPrivKey) {
  ...
  const workersResult = await _workers.Workers.asyncClassMethod(Ecies, 'bitcoreDecrypt', args);
  return workersResult.resbuf;
};