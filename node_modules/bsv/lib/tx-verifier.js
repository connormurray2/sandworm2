...
class TxVerifier extends _struct.Struct {
  ...
  checkStr() {
    ...
    if (this.tx.toBuffer().length > _block.Block.MAX_BLOCK_SIZE) {
      this.errStr = 'transaction over the maximum block size';
      return this.errStr;
    } // Check for negative or overflow output values


    let valueoutbn = new _bn.Bn(0);
    ...
    for (let i = 0; i < this.tx.txOuts.length; i++) {
      ...
      if (txOut.valueBn.gt(_tx.Tx.MAX_MONEY)) {
      ...
      if (valueoutbn.gt(_tx.Tx.MAX_MONEY)) {
  ...
  verifyNIn(nIn, flags) {
    ...
    this.interp = new _interp.Interp();
  ...
  async asyncVerifyNIn(nIn, flags) {
    ...
    const valueBn = txOut.valueBn;
    this.interp = new _interp.Interp();
    const workersResult = await _workers.Workers.asyncObjectMethod(this.interp, 'verify', [scriptSig, scriptPubKey, this.tx, nIn, flags, valueBn]);
